...
public sealed class VanillaPatrolSuppressor
{
    ...
    public void Uninstall()
    {
        try
        {
            ...
            catch (Exception innerEx) { Logger.Warn("VanillaPatrolSuppressor.Uninstall: ClearListeners failed", innerEx); }
            Instance = null;
            _initialized = false;
            Logger.Info("VanillaPatrolSuppressor: uninstalled (listeners cleared, Instance null)");
        }
        catch (Exception ex)
        {
            Logger.Error("VanillaPatrolSuppressor.Uninstall failed", ex);
        }
    }
