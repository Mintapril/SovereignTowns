...
///   - 回到首府后把所有非英雄兵员转入 B 池 Roster，然后自销毁（不调 DefaultMergeAndDisband）。
///
/// SaveableField 槽位：基类占 [10, 20)；本类占 [20, ∞)。
/// 当前字段：20=_troopId，21=_targetCount，22=_targetVillage，23=_phase。
/// </summary>
public sealed class ReserveRecruiterPartyComponent : StPartyComponent
{
    ...
    public override TextObject Name
    {
        get
        {
            ...
            var n = new TextObject("{=ST_BPoolRecruiterName}B-Pool Recruiter — {SETTLEMENT}");
    ...
    private ReserveRecruiterPartyComponent(
        ...
        Settlement capital, Settlement targetVillage, string troopId, int targetCount)
    {
        ...
        {
            Logger.Warn($"ReserveRecruiterPartyComponent.Create: invalid args capital={capital?.StringId ?? "null"} village={targetVillage?.StringId ?? "null"} troop={troopId} count={targetCount}");
            return null;
        }
        try
        {
            ...
            {
                Logger.Error($"ReserveRecruiterPartyComponent.Create: capital '{capital.StringId}' has no OwnerClan/Leader");
                return null;
            }
            ...
            var nameObj = new TextObject("{=ST_BPoolRecruiterName}B-Pool Recruiter — {SETTLEMENT}");
            nameObj.SetTextVariable("SETTLEMENT", capital.Name);

            var component = new ReserveRecruiterPartyComponent(
            ...
            if (mobileParty == null)
            {
                Logger.Error($"ReserveRecruiterPartyComponent.Create: CreateParty returned null for '{stringId}'");
                return null;
            }
            ...
            Logger.Info($"ReserveRecruiterPartyComponent: created '{stringId}' (troop={troopId} target={targetCount}) for '{capital.StringId}'");
            return mobileParty;
        }
        catch (Exception ex)
        {
            Logger.Error("ReserveRecruiterPartyComponent.Create failed", ex);
            return null;
        }
    }
    ...
    private void HandleTravelling(MobileParty self, Settlement capital)
    {
        ...
        {
            Logger.Warn($"ReserveRecruiter '{PartyNameFormatter.SafeName(self)}': village invalid/null → returning");
    ...
    private void HandleAtVillage(MobileParty self, Settlement capital)
    {
        ...
        catch (Exception ex) { Logger.Error($"ReserveRecruiter.HandleAtVillage RecruitByTroopId failed", ex); }

        if (recruited > 0)
            RecruitmentCooldown.MarkRecruited(currentSettlement);

        Logger.Info($"ReserveRecruiter '{PartyNameFormatter.SafeName(self)}': recruited {recruited} '{_troopId}' at '{currentSettlement.Name}'");
    ...
    protected override void OnArrivedHome(MobileParty self)
    {
        try
        {
            ...
            if (capital == null)
            {
                Logger.Warn("ReserveRecruiter.OnArrivedHome: capital is null, destroying without transfer");
                Lifecycle.PartyMergeService.Instance?.DestroyAndUntrack(self, "ReserveRecruiter.OnArrivedHome(no capital)", deferIfInMapEvent: false);
                return;
            }

            var bPool = ReservePoolManager.GetPoolStatic(capital);
            var roster = self.MemberRoster;

            if (bPool != null && bPool.IsActive && roster != null)
            {
                // 按 B 池剩余容量转入（不超 ReservePoolCap 硬上限）
                int cap         = SovereignTowns.Configuration.ConfigurationManager.Current?.FiscalAutonomy?.ReservePoolCap ?? 0;
                ...
                for (int i = roster.Count - 1; i >= 0 && headroom > 0; i--)
                {
                    ...
                    {
                        Logger.Warn($"ReserveRecruiter.OnArrivedHome: transfer troop '{element.Character?.StringId}' failed: {ex.Message}");
                    }
                }
                Logger.Info($"ReserveRecruiter '{PartyNameFormatter.SafeName(self)}': transferred {transferred} troops to B-pool of '{capital.StringId}'");
            }
            else
            {
                Logger.Warn($"ReserveRecruiter '{PartyNameFormatter.SafeName(self)}': no active B-pool for '{capital.StringId}', troops lost");
            }
        }
        catch (Exception ex)
        {
            Logger.Error("ReserveRecruiter.OnArrivedHome: transfer phase failed", ex);
        }
        ...
        try
        {
            Lifecycle.PartyMergeService.Instance?.DestroyAndUntrack(self, "ReserveRecruiter.OnArrivedHome", deferIfInMapEvent: false);
        }
        catch (Exception ex)
        {
            Logger.Error("ReserveRecruiter.OnArrivedHome: DestroyAndUntrack failed", ex);
        }
    }
    ...
    private int RecruitByTroopId(MobileParty self, Settlement village, Settlement capital)
    {
        ...
        foreach (var notable in village.Notables)
        {
            ...
            {
                Logger.Warn($"ReserveRecruiter: MaximumIndexHeroCanRecruitFromHero threw: {ex.Message}");
                continue;
            }
            ...
            for (int i = 0; i < volunteerTypes.Length && i <= maxIdx; i++)
            {
                ...
                {
                    Logger.Warn($"ReserveRecruiter: AddElementToMemberRoster failed for '{troop.StringId}': {ex.Message}");
                    continue;
                }
                ...
                if (shouldCharge && costPerRecruit > 0)
                {
                    if (!Economy.ModTreasury.Charge(capital.OwnerClan, ExpenseCategory.RecruiterWage, costPerRecruit,
                            $"bpool_recruit village={village.StringId} troop={_troopId}"))
                    {
                        // rollback
                        if (added)
                        {
                            ...
                            { Logger.Warn($"ReserveRecruiter: rollback failed: {rollbackEx.Message}"); }
                        }
                        break;
                    }
                }
